...
main :: proc() {
	...
	fmt.println("# parsed glyphs:", len(hershey_glyphs))
	...
	for !rl.WindowShouldClose() {
		...
		if !mode {
			...
			y_offset := draw_text(text, loc=rl.Vector2{20.0, 20.0}, glyphs = &hershey_glyphs, size=64, width=2)
			text = "A QUICK BROWN FOX JUMPS OVER A LAZY DOG"
			y_offset += draw_text(text, loc=rl.Vector2{20.0, 20.0 + y_offset}, glyphs = &hershey_glyphs)
			text = "Sphinx of black quartz, judge my vow"
			draw_text(text, loc=rl.Vector2{20.0, 20.0 + y_offset}, glyphs = &hershey_glyphs)
		}
